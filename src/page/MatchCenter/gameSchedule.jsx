...
const firstFiltering = games.map((el) => {
...
const secondFiltering = selectedMonth === '전체' ? firstFiltering : firstFiltering.filter(obj => obj.hasOwnProperty(selectedMonth));
    ...
    return (
        ...
        <section className="gameScheduleArea size1442">
            ...
            <Games data={secondFiltering} currentYear={currentYear}  />
        </section>
        </>
    )
}
        ...
        <>
            {
                ...
                return (<div key={`month-${index}`} className="games">
                    {/* 1. 월은 데이터의 배열의 상태에따라서 달라지기 때문에 objectkey 를 활용 */}
                    {/* 2. 연도는 현재의 연도가 가장 최상위에 항상 있어야 하기 때문에 datefns 를 사용한 date객체의 year 값을 사용 */}
